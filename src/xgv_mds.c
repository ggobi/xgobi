...
Lp_distance_pow(int i, int j)
{
  ...
  if(mds_lnorm == 2. && mds_distpow == 1.) {
    for (k = 0; k < mds_dims; k++) 
      dsum += (pos.data[i][k]-pos.data[j][k])*(pos.data[i][k]-pos.data[j][k]);
    return(sqrt(dsum));
  } else { /* non-Euclidean or distance power != 1. */
    for (k = 0; k < mds_dims; k++) 
      dsum += pow(fabs(pos.data[i][k]-pos.data[j][k]), mds_lnorm);
...
set_weights()
{
  ...
  {
    if(weights == NULL) /* power weights */
      weights = (double *) XtMalloc(ndistances * sizeof(double));
...
isotonic_transform()
{
   ...
   * could change while their number remains the same...; needs thought.)  
   */
...
mds_once(Boolean doit)
{
  ...
  if(point_status == NULL)
    point_status = (int *) XtMalloc(pos.nrows * sizeof(int)); 
  /* out */
  for(i=0; i<pos.nrows; i++) 
  ...
  for (i = 0; i < dist.nrows; i++) {
    /* do not exclude moving i's: in nonmetric MDS it matters what the
       set of distances is!  */
